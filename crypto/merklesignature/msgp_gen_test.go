...
func TestMarshalUnmarshalSnarkFriendlySignature(t *testing.T) {
	partitiontest.PartitionTest(t)
	v := SnarkFriendlySignature{}
	bts := v.MarshalMsg(nil)
	left, err := v.UnmarshalMsg(bts)
	if err != nil {
		t.Fatal(err)
...
func TestRandomizedEncodingSnarkFriendlySignature(t *testing.T) {
	protocol.RunEncodingTest(t, &SnarkFriendlySignature{})
}

func BenchmarkMarshalMsgSnarkFriendlySignature(b *testing.B) {
	v := SnarkFriendlySignature{}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkAppendMsgSnarkFriendlySignature(b *testing.B) {
	v := SnarkFriendlySignature{}
	bts := make([]byte, 0, v.Msgsize())
	bts = v.MarshalMsg(bts[0:0])
	b.SetBytes(int64(len(bts)))
...
func BenchmarkUnmarshalSnarkFriendlySignature(b *testing.B) {
	v := SnarkFriendlySignature{}
	bts := v.MarshalMsg(nil)
	b.ReportAllocs()
	b.SetBytes(int64(len(bts)))
